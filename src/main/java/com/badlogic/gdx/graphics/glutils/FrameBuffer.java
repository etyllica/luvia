	...
	private void build () {
		...
		if (!defaultFramebufferHandleInitialized) {
			...
			if (Gdx.app.getType() == ApplicationType.iOS) {
				...
				Gdx.gl20.glGetIntegerv(GL20.GL_FRAMEBUFFER_BINDING, intbuf);
		...
		Gdx.gl20.glGenFramebuffers(1, handle);
		framebufferHandle = handle.get(0);

		if (hasDepth) {
			handle.clear();
			Gdx.gl20.glGenRenderbuffers(1, handle);
			depthbufferHandle = handle.get(0);
		}

		Gdx.gl20.glBindTexture(GL20.GL_TEXTURE_2D, colorTexture.getTextureObjectHandle());

		if (hasDepth) {
			Gdx.gl20.glBindRenderbuffer(GL20.GL_RENDERBUFFER, depthbufferHandle);
			Gdx.gl20.glRenderbufferStorage(GL20.GL_RENDERBUFFER, GL20.GL_DEPTH_COMPONENT16, colorTexture.getWidth(),
		...
		if (hasDepth) {
			Gdx.gl20.glFramebufferRenderbuffer(GL20.GL_FRAMEBUFFER, GL20.GL_DEPTH_ATTACHMENT, GL20.GL_RENDERBUFFER, depthbufferHandle);
		}
		...
		if (result != GL20.GL_FRAMEBUFFER_COMPLETE) {
			...
			if (hasDepth) {
				...
				handle.flip();
				Gdx.gl20.glDeleteRenderbuffers(1, handle);
			}

			handle.clear();
			handle.put(framebufferHandle);
			handle.flip();
			Gdx.gl20.glDeleteFramebuffers(1, handle);
